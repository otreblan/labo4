...
\begin{enumerate}[label=\roman*]
	...
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=1, transform shape]
			\begin{axis}[grafico={Descarga $(R_2C=22)$}]
				\addplot[linea] table [volt] {descarga.csv};
				\addplot[linea, blue, domain=12:210, restrict y to domain=0:10] {8*e^((-x+12)/22)};
			\end{axis}
		\end{tikzpicture}
	\end{figure}
...
\subsection{CONCLUSIONES}%
El capacitor puede descargarse un poco aún con el $S_2$ abierto.

Además copiar a mano los datos es una molestia.
Desperdicié mucho tiempo porque no había el logger pro.
